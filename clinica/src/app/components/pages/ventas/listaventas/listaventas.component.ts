...
descargarFactura(venta: any) {
  this.ServicioVentas.getDetalleVentas(venta.venta_id).subscribe((detalles: any[]) => {
    ...
    doc.text(`$${(Number(venta.venta_total) - Number(venta.venta_subiva)  ).toFixed(2)}`, rightMargin, finalY, { align: 'right' });

    // IVA
    doc.text('IVA (15%):', 160, finalY + 6, { align: 'right' });
    doc.text(`$${parseFloat(venta.venta_subiva).toFixed(2)}`, rightMargin, finalY + 6, { align: 'right' });

...
export class listaVentasComponent {
  ...
  descargarFactura(venta: any) {
    this.ServicioVentas.getDetalleVentas(venta.venta_id).subscribe((detalles: any[]) => {
      ...
      const boxHeight = lineHeight * 5; // suficiente espacio
      ...
      doc.setTextColor(100, 100, 100);
      doc.setFont('helvetica', 'normal');
      const calculoIva = Number(venta.venta_subtotal || 0) * (Number(venta.venta_iva || 0) / 100);

      // Subtotal
      doc.text('Subtotal :', 160, finalY + 6, { align: 'right' });
      const subtotal = Number(venta.venta_total || 0) - calculoIva;
      doc.text(`$${subtotal.toFixed(2)}`, rightMargin, finalY, { align: 'right' });

      // IVA
      doc.text('IVA:', 160, finalY + 6, { align: 'right' });
      doc.text(`${Number(venta.venta_iva || 0).toFixed(2)}`, rightMargin, finalY + 6, { align: 'right' });
      // Calculo iva
      doc.text('Calculo Iva:', 160, finalY, { align: 'right' });
      doc.text(`$${calculoIva.toFixed(2)}`, rightMargin, finalY, { align: 'right' });
